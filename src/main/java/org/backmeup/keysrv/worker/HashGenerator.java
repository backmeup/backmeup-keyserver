...
public class HashGenerator
{
	...
	private String calcSHA512 (String value, boolean salted, byte[] salt)
	{
		...
		try
		{
			...
			{	
				...
				System.arraycopy (value_bytes, 0, finaldata, salt.length, value_bytes.length);
				
				byte[] salted_hash = md.digest (finaldata);
				hash = new byte[salt.length + salted_hash.length];
				
				System.arraycopy (salt, 0, hash, 0, salt.length);
				System.arraycopy (salted_hash, 0, hash, salt.length, salted_hash.length);
			}
			else
			{
	...
	/**
	 * The function generates a salted hash out of the given value and compares it to the given hash.
	 * If the hash values are identical the function returns true.
	 * 
	 * @param value
	 * @param hash
	 * @return
	...
	public boolean isCorrectValue (String value, String hash)
	{
		...
		salt = DatatypeConverter.parseHexBinary (hash.substring (0, SALT_LENGTH * 2));
