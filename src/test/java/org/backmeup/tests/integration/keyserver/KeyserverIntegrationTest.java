...
public class KeyserverIntegrationTest extends IntegrationTestBase {
	...
	public void testDeleteUser() {
		...
		when().delete("/users/" + userId).then().statusCode(204);
	}
	...
	public void testRegisterService() {
		...
		try {
			given().header("Accept", "application/json").when()
					.post("/services/" + serviceId + "/register").then()
	...
	public void testDeleteService() {
		...
		when().delete("/services/" + serviceId).then().statusCode(204);
	}
	...
	public void testAddAuthInfos() {
		...
		try {
			...
			given().contentType("application/json")
					.body(jsonAuthInfo.toJSONString()).when()
					.post("/authinfos/add").then().statusCode(204);
		} finally {
			KeyserverUtils.deleteService(serviceId + "");
			KeyserverUtils.deleteUser(userId + "");
		}
	...
	public void testDeleteAuthInfos() {
		...
		try {
			KeyserverUtils.addUser(userId + "", password);
			KeyserverUtils.addService(serviceId + "");
			KeyserverUtils.addAuthInfo(userId, password, serviceId, authInfoId,
					authInfoProperties);

			when().delete("/authinfos/" + authInfoId).then().statusCode(204);
		} finally {
			KeyserverUtils.deleteService(serviceId + "");
			KeyserverUtils.deleteUser(userId + "");
		}
	...
	public void testCreateAccessToken() {
		...
		try {
			...
			given().contentType("application/json")
					.body(jsonTokenRequest.toJSONString()).when()
					.post("/tokens/token").then().statusCode(200).assertThat()
	...
	public void testAccessAuthData() {
		...
		try {
			...
			KeyserverUtils.addAuthInfo(userId, password, serviceId, authInfoId,
					authInfoProperties);
			ValidatableResponse tokenResp = KeyserverUtils.createAccessToken(
					userId, password, serviceId, authInfoId, backupDate,
					reusable, encryptionPwd);

			String tokenId = tokenResp.extract().path("bmu_token_id")
					.toString();
			...
			ValidatableResponse response = given()
					...
					.body(jsonDataRequest.toJSONString()).when()
					.post("/tokens/data").then().statusCode(200).assertThat()
					.body(containsString("authinfos"));

			List<String> authInfoUsernames = response.extract().path(
					"authinfos.ai_data.ai_user");
			List<String> authInfopasswords = response.extract().path(
					"authinfos.ai_data.ai_pwd");
